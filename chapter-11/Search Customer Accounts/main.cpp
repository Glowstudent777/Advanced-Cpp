/*****
 * Author: Glowstudent
 * Program: Customer Accounts + Search Functionality
 *****/
...
void exitProgram();
void clearScreen();
void searchCustomerByName(Customer[], int, const string &);
...
int main()
{
    ...
    while (choice != 9)
    {
        ...
        case 8:
        {
            ...
            cout << "Enter a name or part of a name to search for: ";
            string namePart;
            cin.ignore();
            getline(cin, namePart);

            searchCustomerByName(customers, NUM_CUSTOMERS, namePart);
...
void displayMenu()
{
    ...
    cout << "8. Search for a customer" << endl;
...
void searchCustomerByName(Customer customers[], int NUM_CUSTOMERS, const string &namePart)
{
    bool found = false;

    for (int i = 0; i < NUM_CUSTOMERS; i++)
    {
        if (customers[i].name != "" && customers[i].name.find(namePart) != string::npos)
        {
            if (!found)
            {
                cout << "Matching customers found:\n";
                found = true;
            }
    ...
    if (!found)
    {
        cout << "No customer accounts matched the search criteria.\n";
    }
}
